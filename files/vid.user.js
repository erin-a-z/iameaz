...
// @connect      127.0.0.1
// @updateURL    https://lifauo.com/files/vid.user.js
// @license      GPL
// ==/UserScript==
...
/* 当前用到的快捷键 */
const hasUseKey = {
  keyCodeList: [13, 16, 17, 18, 27, 32, 37, 38, 39, 40, 49, 50, 51, 52, 53, 54, 67, 68, 69, 70, 73, 74, 75, 78, 79, 80, 81, 82, 83, 84, 85, 87, 88, 89, 90, 97, 98, 99, 100, 220],
  keyList: ['enter', 'shift', 'control', 'alt', 'escape', ' ', 'arrowleft', 'arrowright', 'arrowup', 'arrowdown', '1', '2', '3', '4', '5', '6', 'c', 'd', 'e', 'f', 'i', 'j', 'k', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'w', 'x', 'y', 'z', '\\', '|'],
  keyMap: {
    enter: 13,
    shift: 16,
    ctrl: 17,
...
const taskConf = {
      ...
      callback: function (h5Player, taskConf, data) {
        ...
        if (!h5Player[keyName]) {
          ...
          if (h5Player[keyName] === h5Player.playbackRate || h5Player[keyName] === true) {
            if (window.sessionStorage.playbackRate && /(c|x|z|1|2|3|4|5|6)/.test(key)) {
              const curSpeed = Number(window.sessionStorage.playbackRate);
              const perSpeed = curSpeed - 0.1 >= 0 ? curSpeed - 0.1 : 0.1;
              const nextSpeed = curSpeed + 0.1 <= 6 ? curSpeed + 0.1 : 6;
  ...
  'supermd91.cc': {
    fullScreen: 'button.dplayer-icon.dplayer-full-icon',
    webFullScreen: 'button.dplayer-icon.dplayer-full-in-icon'
  },
};
  ...
  ; (async function () {
    ...
    const h5Player = {
      ...
      setVolume: function (num) {
        if (!num) return
        if (typeof window.localSoundVolume == 'undefined') {
          window.localSoundVolume = 1;
        }
        const t = this;
        const player = t.player();
        ...
        const curVol = window.localSoundVolume;
        let newVol = curVol;
        let overHundred = !!(newVol > 1);
        if (!overHundred) {
          if (num > 0) {
            newVol += _num;
            if (newVol > 6) {
        ...
        if (!player.audiocontext) {
          ...
          player.creategain = player.audiocontext.createGain();
          player.source = player.audiocontext.createMediaElementSource(player);
          player.source.connect(player.creategain);
          player.creategain.connect(player.audiocontext.destination);
        }
        if (newVol !== player.creategain.gain.value) {
          player.creategain.gain.value = newVol;
          window.localSoundVolume = newVol;
        }
        /* 条件音量的时候顺便把静音模式关闭 */
        // player.muted = false;

        t.tips(i18n.t('tipsMsg.volume') + parseInt(window.localSoundVolume * 100) + '%');
      ...
      palyerTrigger: function (player, event) {
        ...
        if ((keyCode >= 49 && keyCode <= 54) || (keyCode >= 97 && keyCode <= 100)) {
